...
export class MisComprasDetallesComponent extends HTMLElement {
    ...
    async connectedCallback() {
        ...
        this.idCompra = new URLSearchParams(window.location.search).get("idCompra");
...
`;
    ...
    async #cargarMisCompras() {
        ...
        const comprasFiltradas = this.compras.filter(compra => compra.idCompra === parseInt(this.idCompra));

        if (comprasFiltradas.length === 0) {
            // No se encontraron compras con el idCompra deseado
            return this.#cargarComprasVacio();
        }

        // Devuelve el HTML generado
        return comprasFiltradas.map(compra => {
            return compra.boletos.map((boleto) => {
                return `
                <br><br><br><br>
                <div class="event-container" data-idBoleto="${boleto.idBoleto}" id="masDetalles">
                    <div class="image-event">
                        <img src="/App Web/images/eventos/${boleto.evento.nombreImagen}" alt="">
                    </div>
                    <div class="event-details">
                        <div class="column-event">
                            <p>Costo de Entrada: $${boleto.boleto.precio} MXN</p>
                            <p>Lugar: ${boleto.evento.lugar}</p>
                            <p>Fecha del Evento: ${this.#formatearFecha(boleto.boleto.idEvento_evento.fecha)}</p>
                        </div>
                    </div>
                </div>
                <br><br><br><br>`;
            }).join('');
        }).join(''); // Agrega un join para unir el HTML generado
    }
